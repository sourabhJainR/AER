...
import { AerKind, v, object, table, encodeBinary, decodeBinary, applyPatch, encodeFrame, decodeFrames, validate, dumps } from '../dist/advanced.js';
...
assert.equal(decodeBinary(encodeBinary(sample)).kind, sample.kind);
...
for(let i=0;i<250;i++){const x=object({id:v(AerKind.Int,BigInt(Math.floor(rnd()*100)-50)),ok:v(AerKind.Bool,rnd()>0.5),name:v(AerKind.String,['a','b','a,b'][Math.floor(rnd()*3)])});assert.deepEqual(decodeBinary(encodeBinary(x)),x);}
